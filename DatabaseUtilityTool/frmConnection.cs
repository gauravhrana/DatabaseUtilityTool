...
namespace DatabaseUtilityTool
{
    public partial class frmConnection : Form
    {
        ...
        public frmConnection(string server, string database, string user, string password)
        {
            InitializeComponent();
        ...
        private bool checkConnection()
        {
            try
            {
                ...
                {
                    ((frmApplication)(this.Owner)).ConnectionId = Convert.ToInt32(dt.Rows[0]["ConnectionId"]);
                }
                else
                {
                    ((frmApplication)(this.Owner)).ConnectionId = Connection.Create(data, ApplicationConnectionString);
                }
